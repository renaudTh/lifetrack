...
export class ApiProvider implements ILifetrackApi {
  ...
  public async getHistory(
    ...
    const request = this.http.get<ActivityRecordDTO[]>(
      `api://records?start=${sp}&end=${ep}`,
    );
  ...
  public async recordActivity(
    ...
    const request = this.http.post<ActivityRecordDTO>(`api://record`, {
  ...
  public async downsertRecord(
    record: ActivityRecord,
  ): Promise<ActivityRecord | null> {
    const request = this.http.patch<ActivityRecordDTO | null>(
